<template>
	<view>
		<hx-navbar backTabbarUrl="/pages/gongneng/index" title="课程资源"></hx-navbar>
		<!-- 下拉刷新部分 -->
		<mescroll-body ref="mescrollRef" :down="downOption" @down="downCallback" @up="upCallback">
			<view class=" store-box" v-if="movieList">
				<view class=" item-box" v-for="(movie,i) in movieList" :key="i">
					<view class="container top-box">
						<view class="left" @click="toMovie(movie)">
							<image :src="baseURL+movie.posterUrl" mode="aspectFill"></image>
						</view>
						<view class="right" @click="toMovie(movie)">
							<text class="tit">{{movie.title}}（{{movie.description}}）</text>
							<view class="row justify-content">
								<view class="row-right">
									<i class="hxicon-countdownfill"></i>
									<text>下载次数：{{movie.downloadAmount}}</text>
								</view>
							</view>
						</view>
					</view>
			...
			<view v-if="userType == 1" class="add-btn" @click="toMovie(null)">
				<text>上传课程资源</text>
			</view>
		</mescroll-body>
	...
	export default {
		...
		methods: {
			...
			getMovie() {
				...
				api.ziyuan.getAllMovie(sendData).then(res => {
					if (res.data.code == 0) {
						let params = res.data.data;
						if (Math.ceil(params.total / params.size) < params.current || params.total < params.size) {
							this.NoMore = true;
							this.movieList = [...this.movieList, ...res.data.data.records];
